...
public final class DisjointSets<T> {
    ...
    public Collection<T> retrieveSet(T e) {
        ...
        ArrayList<T> set = new ArrayList<T>(Math.abs(_parents.getInt(root)));
    ...
    public Collection<? extends Collection<T>> retrieveSets() {
        Int2ObjectMap<ArrayList<T>> sets = new Int2ObjectOpenHashMap<ArrayList<T>>(_numSets, 1f);
        ...
        return new ArrayList<ArrayList<T>>( sets.values());
    }
